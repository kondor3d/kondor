...
//Here is the binbox's setting

Servo  hidlight;

//DLP power
//acer H6510BD
//unsigned char power_on[9]={0x56,0x30,0x30,0x53,0x30,0x30,0x30,0x31,0x0D};
//unsigned char power_off[9]={0x56,0x30,0x30,0x53,0x30,0x30,0x30,0x32,0x0D};



//sanlin
//unsigned char power_on[4]={0x30,0x30,0x21,0x0D};
//unsigned char power_off[4]={0x30,0x30,0x22,0x0D};

//LiXun d555
//unsigned char power_on[3]={0x31,0xCE,0x81};
...
SERIAL_PROTOCOLLNPGM("start");
  ...
  #endif
  hidlight.attach(11);
  hidlight.write(servo_close);

  pinMode(triggle,OUTPUT);
  digitalWrite(triggle,LOW);
...
void process_next_command() {
  ...
  switch (command_code) {
    ...
    case 'M': switch (codenum) {
      ...
      case 800:
//        if (code_seen('S')) feedrate_multiplier = code_value();
//        Serial3.write(power_on,3);
          Serial3.write("V00S0001\r");
          digitalWrite(triggle,HIGH);
         SERIAL_PROTOCOLPGM("oMG");
//         SERIAL_PROTOCOLPGM("800");
        break;

      case 801:
//      Serial3.write(power_on,3);
        Serial3.write("V00S0002\r"); 
//      delay(1000);
//      Serial3.write(power_on,3);
      digitalWrite(triggle,LOW);
      ...
      case 802:
        if (code_seen('S')) servo_open = code_value();
        hidlight.write(servo_open);
        SERIAL_PROTOCOLPGM("oMG");
        break;
        
      case 803:
      if (code_seen('S')) servo_close = code_value();
        hidlight.write(servo_close);
        SERIAL_PROTOCOLPGM("oMG");
        break;
      ...
      case 806:
        EEPROM.get(3,keyVar);
        key1 = String(keyVar.key1,DEC);
        key2 = String(keyVar.key2,DEC);
        key3 = String(keyVar.key3,DEC);
        ks = "Pm:"+key1+","+key2+","+key3;
        MYSERIAL.flush();
        MYSERIAL.println(ks);
        MYSERIAL.flush();
        SERIAL_PROTOCOLPGM("oMG");
